...
def _profile(
    *,
    ...
    return RecoilProfileRecord(
        ...
        support_counts=tuple(support_count for _ in samples_y),
        fit_summary={"episode_count": float(burst_count), "accepted_episode_count": accepted_episode_count},
    )
...
class RecoilAuditTests(unittest.TestCase):
    ...
    def test_audit_flags_large_vertical_recovery_tail_without_blocking_runtime_trial(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            profile = _profile(
        ...
        self.assertIn("vertical_recovery_tail", report.profiles[0].findings)
        self.assertTrue(report.profiles[0].runtime_ready)
        self.assertEqual(report.profiles[0].diagnostics["vertical_peak_abs"], 300.0)
        self.assertEqual(report.profiles[0].diagnostics["vertical_recovery_pixels"], 190.0)
        self.assertAlmostEqual(report.profiles[0].diagnostics["vertical_recovery_ratio"], 110.0 / 300.0)
