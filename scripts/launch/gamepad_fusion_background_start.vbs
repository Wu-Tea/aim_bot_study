...
scriptPath = fileSystem.BuildPath(scriptDirectory, "gamepad_fusion_background_start.ps1")
...
If exitCode <> 0 Then
    message = "Fusion background start failed (exit code " & exitCode & ")." & vbCrLf & _
        "See launcher diagnostics:" & vbCrLf & logPath
    shell.Popup message, 0, "Fusion startup failed", 16
End If
