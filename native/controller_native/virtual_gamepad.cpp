...
VirtualGamepad::VirtualGamepad() {
    ...
    if (virtual_gamepad_log_enabled()) {
        std::cout << "[NativeRuntime] ViGEm unavailable; using logging gamepad output.\n";
    }
}
...
void VirtualGamepad::update(const GamepadOutputState& state) {
    if (vigem_) {
        const XusbReport report = to_xusb_report(state);
        vigem_->vigem_target_x360_update(vigem_->client, vigem_->target, report);
        return;
    }
    if (logging_backend_) {
        return;
