...
- July 31 live telemetry and three stationary firing-range videos show the
  remaining defect is not merely noisy boxes: a single 0.2-0.5 s firing burst
  can cause 5-9 requested direction reversals. Remaining-work propagation
  preserves or amplifies the same firing/camera impulse through position and
  velocity. Three local gate/feed-forward experiments were rejected because
  they traded jitter for BodyLock overshoot, lazy tracking or correction tail.
- The accepted July 31 runtime uses a firing-disturbance velocity observer:
  current position remains fresh-Vision authoritative, while only consecutive
  directionally consistent 2-D residuals enter target velocity. BodyLock uses
  it only during firing when the appearance anchor is absent/low-confidence
  and the established target speed is at most 80 px/s. Reliable-anchor and
  normal moving-target paths remain unchanged. LT release now requires about
  24 ms of continuous idle evidence; press remains immediate.

## Next Action

1. User live-accept deployed runtime SHA-256
   `80831B5197B8AC58C20844A349BD673FCC21B2768A055AB4446911257BA70F50`,
   focusing on firing-time horizontal/vertical jitter, close moving targets,
   ADS-to-BodyLock handoff and held-LT continuity.
2. If a defect remains, compare it with the retained horizontal aim-bias and
   body-box-plus-recoil cohorts before changing the 80 px/s evidence boundary.
3. Do not reintroduce the rejected leaky disturbance state, global velocity
   zeroing, final-recoil Remaining ledger or blanket BodyLock observer.
...
- Can a causal two-state observer prevent firing impulses from entering target
  velocity and long-term remaining debt while preserving true reversals?
